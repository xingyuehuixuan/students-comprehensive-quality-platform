...
@Component
@Slf4j
public class JwtTokenStuOrganizationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //1、从请求头中获取令牌
        String token = request.getHeader(jwtProperties.getStuOrganizationTokenName());

        //2、校验令牌
        try {
            log.info("jwt校验:{}", token);
            Claims claims = JwtUtil.parseJWT(jwtProperties.getStuOrganizationSecretKey(), token);
            Long stuOrgId = Long.valueOf(claims.get(JwtClaimsConstant.STU_ORG_ID).toString());
            log.info("当前学生组织id：", stuOrgId);
            BaseContext.setCurrentId(stuOrgId);
            //3、通过，放行
            return true;
        } catch (Exception ex) {
            //4、不通过，响应401状态码
